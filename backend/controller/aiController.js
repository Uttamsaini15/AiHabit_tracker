...
export const weeklyReport = async (req, res) => {
  try {
    ...
    if (!ctx.perHabit.length) {
      return res.json({
        content:
          "You don't have any active habits yet. Create your first habit to start tracking - I'll generate a weekly report once you have some habit data!"
      });
    }
    ...
    const userMsg = `Here is the user's habit data for the past 7 days (${ctx.days[0]} to ${ctx.days[6]}):\n${ctx.perHabit
      .map(
        (h) =>
          `- ${h.name} (${h.category}, ${h.frequency}): completed ${h.completedDays} of the past 7 days, target ${h.targetDays} days/week`
      )
      ...
      suggestions = [
        {
          ...
          category: "Fitness",
          icon: "🚶",
          reason: "Low-friction way to build consistency and improve mood.",
        },
        {
          name: "Read 5 pages",
          description: "Start daily reading to build a leaning routine",
          frequency: "daily",
          category: "Learning",
          icon: "📚",
          reason: "Compounds into significant knowledge over time and improves focus.",
        },
        {
          name: "2 minute of mindful breathing",
